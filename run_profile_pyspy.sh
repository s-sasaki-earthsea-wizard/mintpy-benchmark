...
# Safety guard: cap virtual memory so a runaway profile session is killed
# by the kernel before it can drag the whole machine into OOM-thrash
# territory. Verified incident 2026-05-02: torch.profiler held all CPU
# events for the full patch in memory and reached 94.9 GiB RSS, which
# required a hard reboot. py-spy's own footprint is ~22 MiB — 80 GiB is
# vastly more than needed yet still well below the host's 93 GiB total,
# so the cap only fires under genuine runaway behaviour.
ulimit -v $((80 * 1024 * 1024))  # 80 GiB in KiB
